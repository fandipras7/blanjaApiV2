...
const addDataSeller = ({ id, name, email, password, phoneNumber, storeName, roleId }) => {
  return new Promise((resolve, reject) => {
    pool.query('INSERT INTO seller(id, name, email, phone_number, password , store_name, role_id)VALUES($1, $2, $3, $4, $5, $6, $7)', [id, name, email, phoneNumber, password, storeName, roleId], (err, result) => {
      if (!err) {
        resolve(result)
      } else {
        reject(err)
